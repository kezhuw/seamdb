...
use datafusion::logical_expr::sqlparser::ast::NullsDistinctOption;
...
impl ColumnDescriptorBuilder<'_> {
    ...
    pub fn add_unique_constraint(
        ...
        match primary {
            ...
            false => self.builder.add_unique_index(name, column_names, nulls_distinct),
        }
    }
}
...
impl TableDescriptorBuilder {
    ...
    pub fn add_unique_index(
        ...
        let kind = match nulls_distinct {
            NullsDistinctOption::None | NullsDistinctOption::Distinct => IndexKind::UniqueNullsDistinct,
            NullsDistinctOption::NotDistinct => IndexKind::UniqueNullsNotDistinct,
        };
        let index_meta = IndexMeta::new(kind, name, column_names, &self.descriptor)?;
    ...
    pub fn add_primary_index(&mut self, name: Option<String>, column_names: Vec<String>) -> Result<(), SqlError> {
        ...
        let index_meta = IndexMeta::new(IndexKind::PrimaryKey, name, column_names, &self.descriptor)?;
        self.primary_index = Some(index_meta);
        Ok(())
    }

    pub fn add_index(&mut self, name: Option<String>, column_names: Vec<String>) -> Result<(), SqlError> {
        let index_meta = IndexMeta::new(IndexKind::NotUnique, name, column_names, &self.descriptor)?;
    ...
    pub fn build(mut self) -> Result<TableDescriptor, SqlError> {
        let Some(primary_index) = self.primary_index else {
            return Err(SqlError::invalid(format!("table {} defines no primary index", self.descriptor.name)));
        };
